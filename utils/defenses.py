...
def _get_image_from_arr(img_arr):
    return _Image.fromarray(
        _np.asarray(img_arr, dtype='uint8'))


def denoise_median_filter(img_arr, size=3):
    return _median_filter(img_arr, size=size)


def denoise_tv_bregman(img_arr, weight=30):
    return _np.array(
        _denoise_tv_bregman(img_arr, weight=weight) * 255,
        dtype=img_arr.dtype)
...
def slq_tf(x, qualities=(20, 40, 60, 80), patch_size=8):
    ...
    with tf.name_scope('slq'):
        ...
        patch_n = (n / patch_size) + tf.cond(n % patch_size > 0, lambda: one, lambda: zero)
        patch_m = (m / patch_size) + tf.cond(n % patch_size > 0, lambda: one, lambda: zero)
        ...
        Z = tf.image.resize_nearest_neighbor(
            ...
            (patch_n * patch_size, patch_m * patch_size), name='random_layer_indices')[0, :, :, 0][:n, :m]
        indices = tf.transpose(tf.stack([Z, R, C]), perm=[1, 2, 0], name='random_layer_indices')
