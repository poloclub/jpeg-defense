...
python get_imagenet.py \
  --local_scratch_dir="/path/to/local_scratch_dir"
```

Optionally if the raw data has already been downloaded you can provide a direct
`raw_data_dir` path. If it is provided, it should be in the format:
- Training images: raw_data_dir/train/n03062245/n03062245_4620.JPEG
- Validation Images: raw_data_dir/validation/ILSVRC2012_val_00000001.JPEG
- Validation Labels: raw_data_dir/synset_labels.txt

Conversely, if you have only downloaded the Imagenet TAR files, you can set the
`provision_only` flag to True. Then the script will only provision the dataset.
The TAR files should be available as:
- local_scratch_dir/raw_data/ILSVRC2012_img_train.tar
- local_scratch_dir/raw_data/ILSVRC2012_img_val.tar
"""
...
flags.DEFINE_integer(
    'jpeg_quality', None, 'JPEG compression to be applied to images')
flags.DEFINE_boolean(
    'provision_only', False,
    'Set to true if tar files are already downloaded.')

FLAGS = flags.FLAGS

BASE_URL = 'http://www.image-net.org/challenges/LSVRC/2012/nnoupb/'
LABELS_URL = ('https://raw.githubusercontent.com/'
              'tensorflow/models/master/research/inception/inception/data/'
...
def download_dataset(raw_data_dir):
    ...
    # Check if training and validation tar files exist
    if FLAGS.provision_only:
        assert \
            os.path.exists(os.path.join(raw_data_dir, TRAINING_FILE)) \
            and os.path.exists(os.path.join(raw_data_dir, VALIDATION_FILE)), \
            '%s should contain %s & %s if provision_only flag is set' \
            % (raw_data_dir, TRAINING_FILE, VALIDATION_FILE)

    # Download the training data
    directory = os.path.join(raw_data_dir, TRAINING_DIRECTORY)
    filename = os.path.join(raw_data_dir, TRAINING_FILE)
    if not FLAGS.provision_only:
        tf.logging.info(
            'Downloading the training set. This may take a few hours.')
    ...
    tf.logging.info('Extracting the training set.')
    ...
    if not FLAGS.provision_only:
        tf.logging.info(
            'Downloading the validation set. This may take a few hours.')
        _download(BASE_URL + VALIDATION_FILE, filename)

    tf.logging.info('Extracting the validation set.')
        ...
        if FLAGS.jpeg_quality is not None:
            self._decode_jpeg = tf.image.decode_jpeg(
                tf.image.encode_jpeg(
                    self._decode_jpeg, format='rgb',
                    quality=FLAGS.jpeg_quality), channels=3)
...
def main(argv):  # pylint: disable=unused-argument
    ...
    if FLAGS.provision_only:
        assert FLAGS.raw_data_dir is None, \
            'raw_data_dir should not be set if provision_only is set to True.'
