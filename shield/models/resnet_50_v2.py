...
def _get_updated_endpoints(original_end_points):
    """Adds the keys 'logits' and 'probs' to the end points dictionary of ResNet50-v2.
...
class ResNet50v2(CleverHansModel):
    ...
    Attributes:
        x (tf.Variable): The variable in the tensorflow graph that feeds into the model nodes.
        ...
        default_image_size (int): The image size accepted by the model by default.
        end_points (dict): Dictionary with pointers to intermediate variables of the model.
        variables_to_restore (list): Names of tensorflow variables to be restored in the graph.
    """
